...
foreach ($events as $event) {
    ...
    $startDate = $date->format(lang('global_date_format')) . ' (' . leaveTimeLabel($event['startdatetype']) . ')';
    $objLeave->startDate = $startDate;
    $date = new DateTime($event['enddate']);
    $endDate = $date->format(lang('global_date_format')) . ' (' . leaveTimeLabel($event['enddatetype']) . ')';
    $objLeave->endDate = $endDate;
    $objLeave->cause = $event['cause'];
    $objLeave->duration = formatLeaveDurationHours($event['duration']);
    ...
    $objLeave->duration = ($objLeave->duration==$lastObject->duration)?'':$objLeave->duration;
...
if (!empty($leave)) {
    ...
    $objLeave->startDate = $date->format(lang('global_date_format')) . ' (' . leaveTimeLabel($leave['startdatetype']) . ')';
    $date = new DateTime($leave['enddate']);
    $objLeave->endDate = $date->format(lang('global_date_format')) . ' (' . leaveTimeLabel($leave['enddatetype']) . ')';
    $objLeave->cause = $leave['cause'];
    $objLeave->duration = formatLeaveDurationHours($leave['duration']);
